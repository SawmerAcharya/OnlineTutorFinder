import BookingV2 from "../models/BookingV2.js";
...
export const createBooking = async (req, res) => {
  try {
    ...
    const newBooking = new BookingV2({
      userId: userId,
      tutorId: tutorId,
...
export const getBookingsByTutor = async (req, res) => {
  try {
    ...
    const bookings = await BookingV2.find({ tutorId })
      .populate("userId", "name profile")
      .populate("tutorId", "name")
      .sort({ createdAt: -1 });
...
export const getBookingById = async (req, res) => {
  try {
    ...
    const booking = await BookingV2.findById(bookingId)
...
export const updateBookingStatus = async (req, res) => {
  try {
    ...
    if (status === "payment-received-and-tutor-confirmed") {
      ...
      // Convert daily hours to minutes for Zoom
      const duration = dailyHour * 60;

      const topic = "Tutoring Session";
      // Format start time using Luxon
      const combinedDateTime = DateTime.fromISO(startDate.toISOString(), {
        zone: "utc",
      })
        .setZone("Asia/Kathmandu")
        .set({
          hour: DateTime.fromFormat(updatedBooking.startTime, "h:mm a", {
            zone: "Asia/Kathmandu",
          }).hour,
          minute: DateTime.fromFormat(updatedBooking.startTime, "h:mm a", {
            zone: "Asia/Kathmandu",
          }).minute,
        });

      const startTime = combinedDateTime.toISO(); // ISO string with correct time for Zoom
      const formattedDateTime = combinedDateTime.toFormat(
...
Your recurring Zoom tutoring session has been scheduled.

Topic: ${topic}
Start Date & Time: ${formattedDateTime}
...
`;
        ...
        console.log("Recurring Zoom meeting created:", zoomResult);
      } catch (err) {
...
export const getStudentbyTutor = async (req, res) => {
  try {
    const { tutorId } = req.params;
...
export const removeStudentFromTutorList = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Student removed from tutor's list" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "Server Error" });
  }
  ...
  try {
    ...
    if (booking.status !== "payment-received-and-tutor-confirmed") {
      return res.status(400).json({
        success: false,
        message: "Cannot complete this booking at its current status",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: "Tutor marked booking as completed",
      booking,
    });
  } catch (error) {
    console.error(error);
...
export const closeBookingByStudent = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Tutor must mark the booking as completed first",
      });
    }
    ...
    if (action === "reject") {
      ...
      return res.status(200).json({
        success: true,
        message: "Student rejected booking completion",
        booking,
      });
    }
    ...
    if (action === "confirm") {
      ...
      const transaction = await usersTransactions.findOne({
      ...
      // Update tutor's balance
      const tutorBalance = await balanceModel.findOne({
        userId: booking.tutorId,
      });
      if (tutorBalance) {
        tutorBalance.balance += transaction.amount;
      ...
      return res.status(200).json({
        success: true,
        message: "Student confirmed booking completion",
        booking,
      });
    }
