...
import SlideBars from "./SlideBars"; // Import the SlideBars component
import axios from "axios";
import { toast } from "react-toastify"; // added ToastContainer import
...
export default function WithdrawPage() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          const payment = res.data.userData?.paymentInfo; // <-- changed line
          if (payment) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2) clear inputs & close modal
      resetForm();
      setIsWithdrawModalOpen(false);

      // 3) re-load the updated history
