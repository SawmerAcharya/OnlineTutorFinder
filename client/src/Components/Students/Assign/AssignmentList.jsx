...
import { useEffect, useState, useContext } from "react";
import { Link } from "react-router-dom";
import { Calendar, File, Upload, Check } from "lucide-react";
import axios from "axios";
import { AppContent } from "../../../Context/AppContex";
import { DateTime } from "luxon";
    ...
    <div className="grid gap-6">
      {assignments.map((assignment) => {
        ...
        const submittedAt = assignment.submittedAt
          ? DateTime.fromISO(assignment.submittedAt)
          : null;

        const isPast = dueDate < now;
        const isLate = isPast && !assignment.submitted;
        const isLateSubmitted =
          assignment.submitted && submittedAt && submittedAt > dueDate;
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                <div className="flex items-center gap-1">
                  <Calendar size={16} />
                  <span>Due: {dueDate.toFormat("MMM dd, yyyy")}</span>
                </div>
              </div>
            </div>
...
function StatusBadge({ submitted, isLate, isLateSubmitted }) {
  if (isLateSubmitted) {
    return (
