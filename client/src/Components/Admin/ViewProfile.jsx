...
import { HiUsers } from "react-icons/hi2";
import { RiBook3Line } from "react-icons/ri";
import { IoDocumentTextOutline, IoChevronBackCircleSharp } from "react-icons/io5";
import { toast } from "react-toastify";
...
const TutorProfile = () => {
  ...
  useEffect(() => {
    const fetchTutorProfile = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${backendUrl}/api/user/tutors/${tutorId}`, {
          withCredentials: true,
        });
        if (response.data.success) {
          setTutor(response.data.tutor || {});
        } else {
          throw new Error(response.data.message || "Failed to fetch tutor details.");
        }
      } catch (error) {
        setError(error.response?.data?.message || error.message || "Failed to load tutor profile.");
        toast.error(error.response?.data?.message || "Error fetching tutor details. Try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusUpdate = async (newStatus) => {
    if (!window.confirm(`Are you sure you want to set the status to '${newStatus}'?`)) return;
    ...
    try {
      ...
      if (response.data.success) {
        setTutor(prev => ({ ...prev, status: newStatus }));
        toast.success(`Tutor ${tutor.name}, status updated to: ${newStatus}!`);
        navigate("/tutors");
      } else {
        toast.error(response.data.message || "Failed to update status.");
  ...
  if (error) return <p className="text-center text-red-500">{error}</p>;
  if (!tutor) return <p className="text-center text-gray-500">Tutor not found.</p>;
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-2xl bg-white shadow-lg rounded-xl border border-gray-300 relative">
        ...
        <div className="text-center mt-12">
          <h2 className="text-xl font-semibold text-gray-800">{tutor.name}</h2>
          <p className="text-gray-500 flex items-center justify-center text-sm mt-1">
            <MdLocationOn className="mr-1 text-lg" /> {tutor.tutorData?.City || "N/A"}
          </p>
        </div>

        <div className="flex justify-around py-4 px-8 border-b border-gray-200 bg-gray-50 mt-3">
          <div className="text-center">
            <p className="text-xl font-semibold text-blue-600">{tutor.tutorData?.Experience || "0"}+</p>
            <p className="text-gray-500 text-sm">Years Experience</p>
          </div>
          <div className="text-center">
            <p className="text-xl font-semibold text-purple-600">${tutor.tutorData?.HourlyRate || "N/A"}</p>
            <p className="text-gray-500 text-sm">Monthly Rate</p>
          </div>
        </div>
        ...
        <div className="p-5 bg-blue-50 rounded-lg mx-5 my-3">
          ...
          <ul className="text-gray-600 text-sm space-y-2">
            <li className="flex items-center">
              <SlBadge className="mr-2 text-blue-500" /> {tutor.tutorData?.Qualifications || "N/A"}
            </li>
          </ul>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 px-5">
          <div className="p-3 bg-red-50 rounded-lg border border-red-200">
            <h3 className="font-bold text-lg text-gray-800 mb-2">Contact Information</h3>
            <p className="text-gray-700 flex items-center text-sm">
              <FaPhoneAlt className="mr-2 text-red-500" /> {tutor.tutorData?.PhoneNumber || "N/A"}
            </p>
            <p className="text-gray-700 flex items-center text-sm mt-3">
              <FaEnvelope className="mr-2 text-red-500" /> {tutor.email}
            </p>
          ...
          <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
            <h3 className="font-bold text-lg text-gray-800 mb-2">Teaching Details</h3>
            <div className="flex flex-wrap gap-2">
              <RiBook3Line className="mr-2 text-blue-500 text-lg" />
              <span className="bg-blue-100 text-blue-600 px-3 py-1 text-sm rounded-full flex items-center">
                {tutor.tutorData?.CurrentSubject || "N/A"}
            ...
            <p className="text-gray-600 text-sm mt-3 flex items-center">
              <HiUsers className="mr-2 text-blue-500 text-lg" /> {tutor.tutorData?.TeachingMode || "N/A"}
            </p>
          </div>
        </div>
        ...
        <div className="p-5">
          <h3 className="font-semibold text-lg text-gray-800 mb-2 flex items-center">
            <IoDocumentTextOutline className="text-black-600 mr-2 text-xl" /> Documents
          </h3>
          <div className="border border-gray-300 rounded-lg p-5 text-gray-600 bg-gray-50">
            {documents.length > 0 ? (
              <div className="space-y-2 text-center">
                {documents.map((docId, index) => (
                  <div key={index}>
                    <a
                      href={`https://utfs.io/f/${docId}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:underline"
                    >
